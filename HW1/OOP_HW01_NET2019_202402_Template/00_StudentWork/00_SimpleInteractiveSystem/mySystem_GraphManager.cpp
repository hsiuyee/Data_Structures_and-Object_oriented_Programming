...
//
void  GRAPH_MANAGER::setCurves_IntervalOfX(double min_X, double max_X)
{
	for (int i = 0; i < MAX_NUM_CURVES; ++i) {
		//
		// implement your own stuff
...
//
// mNumCurves must be inside [MIN_NUM_CURVES, MAX_NUM_CURVES]
//
...
void GRAPH_MANAGER::increase_NumOfSamplePoints(int delta_num)
{
	...
	if (MAX_NUM_SAMPLE_POINTS < mNumOfSamplePoints) mNumOfSamplePoints = MAX_NUM_SAMPLE_POINTS;
	setCurves_NumOfSamplePoints(mNumOfSamplePoints);
}
...
void GRAPH_MANAGER::decrease_NumOfSamplePoints(int delta_num)
{
	...
	//
	mNumOfSamplePoints -= delta_num;
	if (mNumOfSamplePoints < MIN_NUM_SAMPLE_POINTS) mNumOfSamplePoints = MIN_NUM_SAMPLE_POINTS;
	setCurves_NumOfSamplePoints(mNumOfSamplePoints);
}
